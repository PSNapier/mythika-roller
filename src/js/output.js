...
function roll() {
  ...
  function getParentData(parentId) {
    let skillsRunes = [...dictionary.skills, ...dictionary.runes].reduce(
      (obj, key) => {
        obj[key] = parseInt(
          document.getElementById(`${parentId}${key.replace(/\s/g, "")}`)
            .value || 0
        );
        return obj;
      },
    ...
    return {
      ...
      geno: document
        ...
        .split("/"),
      genoSecondary:
        document.getElementById(`${parentId}geno`).value.split(" || ").length >
        1
          ? document
              .getElementById(`${parentId}geno`)
              .value.split(" || ")[1]
      ...
      hereditaryTraits: document.getElementById(`${parentId}hereditarytraits`)
        .value,
      ...skillsRunes,
    };
  }
  ...
  function newMythika(mythikaCount) {
    ...
    function rollStatusRank() {
      ...
      if (parent1.rank === "runt" || parent2.rank === "runt") {
        ...
        if (inbred) {
          let x = rng(100);
          status =
            x <= 60 ? "deceased" : x <= 90 ? "inbred" : "inbred - healthy";
        }
        rank = "runt";
      } else if (parent1.rank === "omega" || parent2.rank === "omega") {
        ...
        if (inbred) {
          let x = rng(100);
          status =
            x <= 40 ? "deceased" : x <= 80 ? "inbred" : "inbred - healthy";
        }
        rank = "runt";
      } else if (parent1.rank === "beta" || parent2.rank === "beta") {
        ...
        if (inbred) {
          let x = rng(100);
          status =
            x <= 20 ? "deceased" : x <= 80 ? "inbred" : "inbred - healthy";
        }
        ...
        if (inbred) {
          let x = rng(100);
          status =
            x <= 10 ? "deceased" : x <= 60 ? "inbred" : "inbred - healthy";
        }
    ...
    function rollBuild() {
      ...
      buildProbabilities.forEach(([rarity1, rarity2, probability]) => {
        if (parent1Build.rarity === parent2Build.rarity) {
          ...
          build =
            rng(100) <= probability ? parent2Build.gene : parent1Build.gene;
        } else if (
          parent1Build.rarity === rarity2 &&
          parent2Build.rarity === rarity1
        ) {
          build =
            rng(100) <= probability ? parent1Build.gene : parent2Build.gene;
        }
      });
    ...
    function rollGeno(parent1Geno, parent2Geno) {
      ...
      return [
        coat,
        ...markings.filter(onlyUnique).sortByArray(dictionary.markingsSorted),
      ]
        .filter(Boolean)
        .join("/");
    }
    ...
    function rollHereditaryTraits() {
      ...
      [parent1.hereditaryTraits, parent2.hereditaryTraits].forEach((trait) => {
        let rarity = Object.keys(dictionary.hereditaryTraits).find((key) =>
