...
function roll() {
	...
	function getParentData(parentId) {
		let skillsRunes = [...dictionary.skills, ...dictionary.runes].reduce(
			(obj, key) => {
				obj[key] = parseInt(
					document.getElementById(
						`${parentId}${key.replace(/\s/g, '')}`,
					).value || 0,
				);
				return obj;
			},
		...
		return {
			bloodline: document
				.getElementById(`${parentId}bloodline`)
				.value.replace(/\,/g, '')
				.split(' || ')[0]
				.split(' '),
			geno: document
				.getElementById(`${parentId}geno`)
				.value.split(' || ')[0]
				.split('/'),
			genoSecondary:
				document
			...
			bonusTrait: document.getElementById(`${parentId}bonustrait`)
				.value,
			mutation: document.getElementById(`${parentId}mutation`).value,
			hereditaryTraits: document.getElementById(
				`${parentId}hereditarytraits`,
			).value,
			...skillsRunes,
		};
	}
	...
	function getSelections() {
		return {
			...
			arativasSpirit:
				document.getElementById(`arativasspirit`).checked,
			...
			toleranceCrystal:
				document.getElementById(`tolerancecrystal`).checked,
			...
			soulApocalypse:
				document.getElementById(`soulapocalypse`).checked,
			mutationKingsAssistant: document.getElementById(
				`mutationkingsassistant`,
			).checked,
			unknownMixtureA:
				document.getElementById(`unknownmixturea`).checked,
			unknownMixtureB:
				document.getElementById(`unknownmixtureb`).checked,
		};
	}
	...
	function newMythika(mythikaCount) {
		...
		function rollSpecies() {
			...
			[parent1, parent2].forEach((parent) => {
				let parentTrait = parent.hereditaryTraits.replace(
					' DNA I',
					'',
				);
				if (dictionary.species.includes(parentTrait)) {
					parent.species =
						rng(100) <= 60 ? parentTrait : parent.species;
				}
			});
			...
			function checkParents(a, b) {
				...
				for (
					let k = 0;
					k < dictionary.speciesIllegalCrosses.length;
					k++
				) {
					if (
						a !== b &&
						dictionary.speciesIllegalCrosses[k].includes(a) &&
						dictionary.speciesIllegalCrosses[k].includes(b)
					) {
				...
				if (
					(parent1.species === a && parent2.species === b) ||
					(parent1.species === b && parent2.species === a)
				) {
					return true;
				}
				return false;
			...
			main: for (let i = 0; i < dictionary.species.length; i++) {
				if (
					checkParents(
		...
		let behophoenixSavedThisPup = false;
		let mutationFromKingsAssistant = false;
		let currentStatus = '';
		function rollStatusRank() {
			...
			if (status === 'inbred') {
				...
				if (mutation === 'harlequin') harlequin = true;
		...
		function rollBuild() {
			...
			buildProbabilities.forEach(([rarity1, rarity2, probability]) => {
				if (parent1Build.rarity === parent2Build.rarity) {
					...
					parent1Build.rarity === rarity1 &&
					parent2Build.rarity === rarity2
				) {
					build =
						rng(100) <= probability
							? parent2Build.gene
					...
					parent1Build.rarity === rarity2 &&
					parent2Build.rarity === rarity1
				) {
					build =
						rng(100) <= probability
							? parent1Build.gene
			...
			return build !== ''
				? `${build.capitalizeStr()} Build`
				: 'Natural Build';
		}

		function rollPhysical() {
			let parentEarTrait = randomizer(
				[parent1.earTrait, parent2.earTrait].filter(Boolean),
			);
			let ears = rng(100) <= 10 ? parentEarTrait : '';

			let parentTailTrait = randomizer(
				[parent1.tailTrait, parent2.tailTrait].filter(Boolean),
			);
			let tail = rng(100) <= 10 ? parentTailTrait : '';

			let parentBonusTrait = randomizer(
				[parent1.bonusTrait, parent2.bonusTrait].filter(Boolean),
			);
			let bonus = rng(100) <= 10 ? parentBonusTrait : '';

			return [ears, tail, bonus]
				.filter(Boolean)
				.join(', ')
				.capitalizeStr();
		}
		...
		function rollMutation() {
			if (currentStatus.startsWith('inbred - ')) {
				return '';
			}
			let output = '';
			...
			if (parent1.mutation === '' && parent2.mutation === '') {
				if (healthy) {
					output =
						rng(100) <= 5 + extraPass
							? randomizer(dictionary.mutations.random)
							: '';
				} else {
					output =
						rng(100) <= 5 + extraPass
							? randomizer(dictionary.mutations.inbred)
							: '';
				}
				...
				output =
					rng(100) <= 8 + extraPass
						? randomizer([parent1.mutation, parent2.mutation])
						: '';
			...
			if (
				output !== '' &&
				selections.mutationKingsAssistant &&
				(output === 'chimera' || output === 'harlequin')
			) {
				mutationFromKingsAssistant = true;
			}
			if (
				output !== 'chimera' &&
				output !== 'harlequin' &&
		...
		function rollGeno(parent1Geno, parent2Geno) {
			...
			coatProbabilities.forEach(([rarity1, rarity2, probability]) => {
				if (parent1Coat.rarity === parent2Coat.rarity) {
					...
					parent1Coat.rarity === rarity1 &&
					parent2Coat.rarity === rarity2
				) {
					coat =
						rng(100) <= probability
							? parent2Coat.gene
					...
					parent1Coat.rarity === rarity2 &&
					parent2Coat.rarity === rarity1
				) {
					coat =
						rng(100) <= probability
							? parent1Coat.gene
			...
			function rollMarkings(parentGeno) {
				for (let key in dictionary.markings) {
					dictionary.markings[key].forEach((marking) => {
						if (
							parentGeno.includes(marking[1]) &&
							shouldPushMarking(key)
						) {
			...
			return [
				...
				...markings
					.filter(onlyUnique)
					.sortByArray(dictionary.markingsSorted),
			]
				.filter(Boolean)
				.join('/');
		}
		...
		function rollHereditaryTraits() {
			...
			[parent1.hereditaryTraits, parent2.hereditaryTraits].forEach(
				(trait) => {
					let rarity = Object.keys(
						dictionary.hereditaryTraits,
					).find((key) =>
						dictionary.hereditaryTraits[key].includes(trait),
					);
		...
		if (behophoenixSavedThisPup) output += `\nBehophoenix saved this pup`;
		if (mutationFromKingsAssistant)
			output += `\nMutation from King's assistant`;
	...
	if (inbred) {
		inbredAlert.innerHTML = `This combination has resulted in inbreeding through ${inbredIds.join(
			', ',
		)}, which can result in mutations and birth defects, or even death.`;
	} else {
	...
	if (selectionsUsed.filter(Boolean).length > 0) {
		...
		used.innerHTML = `Breeding used ${selectionsUsed
			.filter(onlyUnique)
			.join(', ')
			.capitalizeStr()}`;
